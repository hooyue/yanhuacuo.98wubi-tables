...
- `识别码/`：派生维护数据。`去识别码-重码报告.md`（2026-09-13 多义表去识别码重编码的规则、重码率前后对比、特殊设定行去向）与 `去识别码-驱逐与变动明细.txt`（驱逐/新增/消失行全量清单）。`识别码-逐字标注.txt`、`三位行-分类清单.txt` 为**改动前**（28,469 行版）快照：现行多义表已去识别码，全码为 2/3/4 位。判定基准：拆分母表拆分列末位的 15 个专用识别码 PUA 字形（键位映射见目录内 readme.md），勿按末位字母臆测。
